...
function SearchBar() {
  ...
  const throttledFetch = useCallback(
    debounce(() => searchStore.fetchSuggestions(), 1000),
    []
  );
  ...
  const animate =
    searchStore.show &&
    (searchStore.bookResults.length > 0 || searchStore.userResult != null)
      ? "active"
      : "inactive";
  return (
    <Search
    ...
    <AnimatePresence>
      {searchStore.show && (
        <SearchResult
          ...
          animate="show"
        >
          {searchStore.bookResults.map((book) => (
            <SearchBarBook
          ...
          {searchStore.userResult && (
            <UserBook
              onClick={(e) => searchStore.setShow(false)}
              role={searchStore.userResult.role}
              key={searchStore.userResult.username}
              username={searchStore.userResult?.username}
            />
          )}
        </SearchResult>
      )}
...
const UserBook = styled(UserResultBook)({
  width: "95%",
